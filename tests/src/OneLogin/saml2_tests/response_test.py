...
""".strip()]
    ...
    def testIsInValidCert(self):
        ...
        """
        settings_info = self.loadSettingsJSON()
        settings_info['idp']['x509certs'] = 'NotValidCert'
        settings = OneLogin_Saml2_Settings(settings_info)
        xml = self.file_contents(join(self.data_path, 'responses', 'valid_response.xml.base64'))
        response = OneLogin_Saml2_Response(settings, xml)
    ...
    def testIsValid2(self):
        ...
        idp_certs = settings_info_3['idp']['x509certs']
        settings_info_3['idp']['certFingerprint'] = OneLogin_Saml2_Utils.calculate_x509_fingerprint(idp_certs[0])
        settings_info_3['idp']['x509certs'] = []
        settings_3 = OneLogin_Saml2_Settings(settings_info_3)
        response_3 = OneLogin_Saml2_Response(settings_3, xml_2)
        response_3.is_valid(self.get_request_data())
        ...
        settings_info_3['idp']['certFingerprint'] = OneLogin_Saml2_Utils.calculate_x509_fingerprint(idp_certs[0], 'sha256')
